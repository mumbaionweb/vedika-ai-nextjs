  ...
  return (
    <>
      ...
      >
        ...
        <div className="p-4">
          ...
          <div 
            ...
            onMouseEnter={() => {
              ...
              // The logic to show history is now handled automatically
            }}
            onMouseLeave={() => {
          ...
          >
            <button 
              ...
              title="Chat"
              onClick={() => {
                // On mobile, this button can toggle the history view if needed
                // For now, it does nothing as history is always loaded
              }}
            >
              <svg
                className="w-6 h-6 text-primary-400 group-hover:text-primary-300 transition-colors"
            ...
            {/* Chat History Submenu - Now always rendered on desktop, shown via CSS */}
            <div className="hidden lg:block absolute left-full top-0 -ml-1 w-56 bg-white rounded-lg shadow-2xl border border-gray-100 z-50 overflow-hidden group-hover:block">
              {/* Submenu Header */}
        ...
        {/* Middle Section - Navigation/Chat History (for mobile) */}
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 lg:hidden">
          <div className="space-y-2">
            <h3 className="text-sm font-semibold text-secondary-700 px-2 mb-2">Chat History</h3>
            {loadingHistory ? (
              <div className="px-2 py-4 text-center">
                <div className="inline-block animate-spin rounded-full h-5 w-5 border-b-2 border-primary-600"></div>
                <p className="text-xs text-secondary-500 mt-2">Loading...</p>
              </div>
            ) : chatHistory.length === 0 ? (
              <div className="px-2 py-4 text-center">
                <p className="text-xs text-secondary-500">No conversations yet</p>
              </div>
            ) : (
              chatHistory.map((chat) => (
                <div
                  key={chat.conversation_id}
                  className="relative group/item px-2 py-2.5 rounded-lg hover:bg-primary-100 transition-all cursor-pointer"
                  onClick={() => handleChatClick(chat.conversation_id)}
                >
                  <div className="pr-6">
                    <p className="text-sm font-medium text-secondary-900 truncate">{chat.title || chat.topic || 'Untitled'}</p>
                    <p className="text-xs text-secondary-500 mt-1">{formatTimestamp(chat.updated_at)}</p>
                  </div>

                  {/* Three Dots Menu */}
                  <button
                    data-three-dots
                    className="absolute right-1 top-1/2 -translate-y-1/2 p-1 rounded-full hover:bg-white"
                    onClick={(e) => {
                      e.stopPropagation();
                      setActiveOptionsMenu(activeOptionsMenu === chat.conversation_id ? null : chat.conversation_id);
                    }}
                  ...
                  >
                    <svg
                      className="w-4 h-4 text-secondary-500 hover:text-secondary-700"
                      fill="currentColor"
                      viewBox="0 0 24 24"
                    >
                      <path d="M12 8c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z" />
                  ...
                  {activeOptionsMenu === chat.conversation_id && (
                    <div 
                      data-options-menu
                      className="absolute right-0 top-full mt-1 bg-white rounded-md shadow-xl border border-gray-200 py-1 z-30 min-w-[120px]"
                      onClick={(e) => e.stopPropagation()}
                    >
                      <button
            ...
            {chatHistory.length > 0 && (
              <div className="pt-2 mt-2 border-t border-primary-200">
                <button
                  onClick={handleViewAll}
                  className="w-full text-center text-sm font-semibold text-primary-600 hover:text-primary-700 transition-colors py-2 rounded-lg hover:bg-primary-100"
                >
                  View All
                </button>
              </div>
